...
def ProDuSe(fastq1: Path, fastq2: Path, reference: Path, output: str, bwa: Executable, samtools: Executable, bed: PathOrNone = None, verbose: bool=False):
    ...
    if verbose:
        ...
        os.set_inheritable(w, True)
        debugPipes.append(open(r, 'rb'))
    clipProc = Process(target=clip, args=(clipIn, sort.stdin), kwargs=dict(alternate=True, trimTail=True, verbose=verbose, logStream=open(w if verbose else os.devnull, 'w', buffering=1)))
    clipProc.start()
    clipIn.close()
    sort.stdin.close()
