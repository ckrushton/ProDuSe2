...
class FamilyRecord:
    ...
    def aggregate(self, record: pysam.AlignedSegment):
        ...
        while recordItr.next():
            if len(self.cols) < i:
                op = self.Op(recordItr.getOp(), recordItr.getSeqBase())
                op += recordItr.getBaseQual() or 0
                self.cols.append(sortedcontainers.SortedListWithKey(key=lambda x: (x.op, x.allele), iterable=[op]))
            else:
                self.cols[i][(recordItr.getOp(), recordItr.getSeqBase())] += recordItr.getBaseQual() or 0
            i += 1
